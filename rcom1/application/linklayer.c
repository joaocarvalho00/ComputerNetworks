...
void atende()                   // atende alarme
{
    ...
    if(flag == 0)
    {
      ...
      tcflush(fd, TCIOFLUSH);
	    int res2 = write(fd,frame,FrameSize);   
...
int llread(char* packet)
{
  ...
  unsigned int destuffedframeSZ = 0;
  ...
  while (1)
  {
    while (1) 
    {
      ...
      else if(state==2 && ((char_read==C0 && R==0)||(char_read==C1 && R==1))) {
        state=5;
        repeat=1;
        printf("message already recived, discarding ...\n");
      }
    ...
    printf("%d READ\n",FrameSize);
    destuffedframe = (unsigned char*)calloc(FrameSize,sizeof(unsigned char));
    destuffedframeSZ = FrameSize;
    int i, j=0;
    ...
    dataSZ= destuffedframeSZ-4-2;

    data = (unsigned char*)calloc(dataSZ,sizeof(unsigned char));
    //printf("teste 1\n");
